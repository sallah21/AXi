...
parameter MR_IDLE	= 3'b000;
parameter MR_ADDR	= 3'b001;
parameter MR_AACK	= 3'b011;
parameter MR_DATA	= 3'b100;
parameter MR_DACK	= 3'b110;
...
always @(posedge CLK) begin
	...
	if(EN) begin
		...
		BUF_CTRL[BUF_IDLE]: begin
			if(BUF_EN & IMG_EN)
				BUF_CTRL <= (1 << BUF_INIT);
		end
		BUF_CTRL[BUF_INIT]: begin
			BUF_CTRL <= (1 << BUF_CALC_L);
			RD_PTR <= IMG_BUF;
			//RD_CNT <= 153_600; //640 x 480 / 2 (16-bit)
			RD_CNT <= IMG_LEN;
		end
		BUF_CTRL[BUF_CALC_L]: begin
			BUF_CTRL <= (1 << BUF_T_RQ);
			if(|RD_CNT[17:4])
		...
		BUF_CTRL[BUF_T_RQ]: begin
			if(MR_CTRL == MR_IDLE) begin
				BUF_CTRL <= (1 << BUF_CALC_A);
			end
		end
		BUF_CTRL[BUF_CALC_A]: begin
			BUF_CTRL <= (1 << BUF_CONT);
			RD_PTR[31:2] <= RD_PTR[31:2] + RD_RQ_LEN + 1'b1;
			RD_CNT <= RD_CNT + {{14{1'b1}}, ~RD_RQ_LEN};
		end
		BUF_CTRL[BUF_CONT]: begin
			if(~BUF_FULL) begin
				if(|RD_CNT)
					BUF_CTRL <= (1 << BUF_CALC_L);
				else
